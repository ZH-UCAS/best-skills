...
| 用途                          | Skill               | 示例 Prompt                            |
| --------------------------- | ------------------- | ------------------------------------- |
| 读取 / 提取（含 .doc 老格式打不开）      | office-file-process | 「读取这个 word 文档」「提取 excel 内容」「看这个 ppt 讲了什么」 |
| 创建 / 编辑                      | office-file-process | 「生成 Word 报告/合同」「制作幻灯片/deck」「写带公式的电子表格」   |
| 转换 / 校验                      | office-file-process | 「把这个 pptx 转成 markdown」「检查生成的文档有没有问题」   |


> **office-file-process**：Word / Excel / PowerPoint 一站式处理。读取优先 pandoc / markitdown 等官方工具，环境没有则走 Windows Office COM 自动化兜底（含中文文件名、PowerShell 编码等 Windows 方案）；创建 / 编辑用 docx-js、pptxgenjs、openpyxl 等。
...
| 用途                 | Skill | 示例 Prompt                          |
| ------------------ | ----- | ---------------------------------- |
| 提取文本 / 表格 / 图片      | pdf   | 「提取这个 PDF 里的表格」「把 PDF 内容转成文本」       |
| 合并 / 拆分 / 旋转 / 加水印  | pdf   | 「合并这几个 PDF」「给 PDF 加水印」「只要第 3-5 页」  |
| 填表单 / 加解密           | pdf   | 「填写这个 PDF 表单」「给 PDF 设置密码」           |
| 扫描件 OCR            | pdf   | 「这个 PDF 是扫描件，OCR 一下让它能搜索」           |


> **pdf**：PDF 全流程处理，覆盖读取提取、合并拆分、页面操作、表单填写、加解密、图片提取与扫描件 OCR。
